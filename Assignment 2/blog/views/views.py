...
from blog.cache_util import comments_cache_invalidate
...
def post_one(request: HttpResponse, post_id):
    ...
    if request.method == 'POST':
        ...
        comment.post = post
        comment.save()
        comments_cache_invalidate(post_id)
    return render(request, 'post_one.html',
                  {'post': post, 'form': CommentForm(), 'comments': Comment.objects.filter(post__pk=post_id)})
